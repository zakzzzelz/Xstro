...
/**
 * Parses a time string (e.g., "2:30pm", "3:09am") and returns the duration in seconds.
 * @param {string} timeString - The time string to parse.
 * @returns {number|null} The duration in seconds, or null if the format is invalid.
 */
export function parseTime(timeString) {
  ...
  if (muteTime < now) muteTime.setDate(muteTime.getDate() + 1);

  return Math.floor((muteTime - now) / 1000); // Return the duration in seconds
}
