import { copyFileSync, readdirSync, statSync, mkdirSync, writeFileSync, existsSync } from 'fs';
...
const moveFiles = async (sourceFolder, destinationFolder) => {
	const processDirectory = dirPath => {
		const items = readdirSync(dirPath);

		for (const item of items) {
			const sourcePath = join(dirPath, item);
