...
import { loadMessage } from '../../plugins/sql/store.js';
import { getAnti } from '../../plugins/sql/antidel.js';
		...
		if (update.key && (update.update.deleteMessage || update.update?.message === null)) {
			...
			if (store && store.message) {
				...
				if (isGroup) {
					...
					const senderNumber = jidNormalizedUser(msg.key.participant.split('@')[0]);
					const deleterNumber = jidNormalizedUser(update.key.participant.split('@')[0]);
