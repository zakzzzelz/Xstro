...
export default async function useSQLiteAuthState(sessionId) {
	...
	await AuthStateModel.sync({ force: true });
	...
	const updateData = async (key, newData) => {
		const serialized = JSON.stringify(bufferToJSON(newData));
		const [updatedRows] = await AuthStateModel.update({ data_value: serialized }, { where: { session_id: sessionId, data_key: key } });
