...
export async function handleCommand(cmd, msg, conn, __patch) {
  ...
  try {
    ...
    if (mode && !sudo) return;
    if (banned)
      return await __patch.sendReply(
        '```@' + userId + ' you have been banned from using commands```',
        { mentions: [numtoId(userId)] }
      );
    if (!mode && !cmd.isPublic && !sudo)
      return await __patch.sendReply('```@' + userId + ' this command is for my owners```', {
        mentions: [numtoId(userId)],
      });
