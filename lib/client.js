...
import { handleAntiDelete } from './events/antidelete.js';
import { fancy } from '../plugins/client/font.js';
import { handleGroupParticipants } from './events/greetings.js';
import { handleViewOnce } from './events/antivv.js';
import { chatAi } from './events/lydia.js';
import { handleAutoKick } from './events/autokick.js';
import { schedules } from './events/scheduler.js';
...
const connect = async () => {
	...
	conn.ev.on('messages.upsert', async ({ messages, type }) => {
		if (type === 'notify') {
			...
			await chatAi(__msg, conn)
			await handleViewOnce(__msg, conn, __class);
			if (__msg.from && __msg.body && __msg.isGroup) await handleAntiWord(conn, __msg);
			if (__msg.from && __msg.body && __msg.isGroup) await handleAntiLink(conn, __msg);
			if (__msg.from && __msg.isGroup) handleAutoKick(conn, __msg)
			await handleMessage(__msg, conn, cache, __class);
		}
	});
	conn.ev.on('messages.update', async updates => { await handleAntiDelete(conn, updates); });
