...
import { proxy, manageProcess, deepClone } from '#utils';
import { AntiCall, Greetings, GroupEventPartial, GroupEvents } from '#bot';
import { loadMessage, saveMessage, getGroupMetadata, getName, getConfig } from '#sql';
import { upserts, Plugins, serialize, logger } from '#lib';
...
export const client = async () => {
  ...
  conn.ev.process(async (events) => {
    ...
    if (events['messages.upsert']) {
      ...
      // console.log(JSON.parse(JSON.stringify(messages[0]?.message?.protocolMessage, null, 2)));
      if (type !== 'notify') return;

      const { autoRead, autoStatusRead, autolikestatus } = await getConfig();

      for (const message of messages) {
        const msg = await serialize(deepClone(JSON.parse(JSON.stringify(message, null, 2))), conn);
        console.log(msg);
