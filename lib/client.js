...
import { WORK_TYPE, AUTO_READ, AUTO_STATUS_READ, LOGS } from "../config.js";
	...
	conn.ev.on("messages.upsert", async ({ messages, type }) => {
		...
		for (const cmd of commands) {
			const execCmd = async (args) => {
				const msgInstance = new Message(conn, msg);
				try {
					if (cmd.function.constructor.name === "AsyncFunction") {
						await cmd.function(msgInstance, ...args, msg, conn, messages);
					} else {
						cmd.function(msgInstance, ...args, msg, conn, messages);
					}
				} catch (err) {
