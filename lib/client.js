...
import { isLatest, getRandomProxy, manageProcess, numtoId } from '#utils';
import { getConfigValues, upserts, commands, Plugins, serialize, logger, devs } from '#lib';
...
export const client = async () => {
	...
	const logo = await readFile('./media/xstro.jpg');
	...
	sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
		if (connection === 'close') {
			...
			for (const dev of devs) await sock.sendMessage(numtoId(dev), { text: '```Bot Connected```' });
