...
import { MODE, AUTO_READ, AUTO_STATUS_READ, LOGS, VERSION, SESSION_ID, CMD_REACT, PREFIX } from '../config.js';
import { serialize } from './serialize.js';
import Message from './class/Base.js';
import { saveChat, saveMessage, saveContact } from './db/store.js';
import { fancy } from './utils/font.js';
	...
	const handleMessage = async rawMessage => {
		...
		async function execmd(cmd, msg, conn) {
			...
			try {
				...
				const cmdName = cmd.pattern ? (cmd.pattern.toString().match(/\w+/) || [])[0] || 'unknown' : cmd.on;
