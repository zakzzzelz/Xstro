...
import { numberToJID } from './utils/utils.js';
...
const connect = async () => {
	...
	const handleMessage = async rawMessage => {
		...
		async function executeCommand(cmd, msg, conn) {
			try {
				await cmd.function(new Message(conn, msg), msg.body, msg, conn);
			} catch (err) {
				const errorText = `\`\`\`ERROR: ${cmd.pattern ? cmd.pattern.toString().split(/\W+/)[1] : 'Unknown'} - ${err.message}\`\`\``;
				const senderJID = numberToJID(msg.sender.split('@')[0].split(':')[0]);
				await conn.sendMessage(msg.from, { text: errorText, mentions: [senderJID] }, { quoted: msg });
			}
		}
		...
		for (const cmd of commands) {
			if (msg.body && cmd.pattern) {
				...
				if (match) {
					...
					await executeCommand(cmd, modifiedMsg, conn);
					continue;
				}
			}
			...
			if (cmd.on && msgType) {
				await executeCommand(cmd, msg, conn);
			}
		}
	};
