...
import { handleAntiDelete, storeMessage } from './client/antidelete.js';

const { DisconnectReason } = baileys;
const { MODE, AUTO_READ, AUTO_STATUS_READ, VERSION, PREFIX } = config;
...
const connect = async () => {
	...
	conn.ev.on('messages.upsert', async ({ messages, type }) => {
		if (type === 'notify') {
			...
			await handleMessage(messages[0], conn, patternCache, AUTO_READ, AUTO_STATUS_READ);
		}
	});
