...
export async function serialize(messages, conn) {
	...
	const quoted = messages.message?.[msg.type]?.contextInfo?.quotedMessage || messages.message?.[msg.type]?.contextInfo?.quotedMessage?.ephemeralMessage;
	if (quoted) {
		...
		msg.quoted = {
			...
			body: quotedMessage?.conversation || quotedMessage?.[quotedType]?.text || quotedMessage?.[quotedType]?.caption || false, // Still need to add poll support, i don't think so anymore
		};
	} else {
		msg.quoted = null;
	}
