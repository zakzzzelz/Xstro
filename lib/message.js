...
export async function serialize(messages, conn) {
	...
	const contextInfo = messages.message?.[msg.type]?.contextInfo;
	const quotedRawMessage = contextInfo?.quotedMessage;

	if (quotedRawMessage) {
		const quotedContent = quotedRawMessage.ephemeralMessage?.message || quotedRawMessage;
		if (quotedContent) {
			const quotedMessage = normalizeMessageContent(quotedContent);
			const quotedType = getContentType(quotedMessage);

			msg.quoted = {
				key: {
					id: contextInfo.stanzaId,
					fromMe: contextInfo.participant === owner,
					remoteJid: isGroup ? remoteJid : jidNormalizedUser(contextInfo.participant),
				...
				isEphemeral: !!quotedRawMessage.ephemeralMessage,
