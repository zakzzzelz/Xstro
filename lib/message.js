...
export async function smsg(messages, conn) {
	...
	if (messages.message) {
		...
		Object.assign(msg, {
			...
			body: normalizedMessage?.[type]?.editedMessage?.conversation || normalizedMessage?.[type]?.editedMessage?.text || normalizedMessage?.[type]?.editedMessage?.caption || normalizedMessage?.conversation || normalizedMessage?.[type]?.text || normalizedMessage?.[type]?.caption || false,
		});
	}
