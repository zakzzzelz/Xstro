...
export async function serialize(messages, conn) {
	...
	// To Do 
// Fix no inclusion of ephemeralMessage quoted
	if (quoted) {

// I don't know i thought it was normalized
		const quotedMessage = normalizeMessageContent(quoted);
// This may cause some cmds, like, vv, save, forward, and gc commands not to work
		const quotedType = getContentType(quotedMessage);
		console.log(quotedType) // Pre checks required
		msg.quoted = {
			key: {
			...
			message: quotedMessage, // Needs more work
			type: quotedType, // Needs more work for more accuarcy
			viewonce: quotedMessage?.[quotedType]?.viewOnce || false,
			body:
				quotedMessage?.conversation ||
				quotedMessage?.[quotedType]?.text ||
				quotedMessage?.[quotedType]?.caption ||
				false, // Still need to add poll support
		};
	} else {
		msg.quoted = false; // null not a false
	}
