...
export async function flipMedia(buffer, direction) {
	...
	const res = await axios.post(`${config.XSTRO_API}/api/flip?direction=${direction}`, form, {
...
export async function toBlackVideo(buffer, color = 'black') {
	...
	const response = await axios.post(`${config.XSTRO_API}/api/blackvideo`, form, {
		headers: {
			...form.getHeaders(),
		},
		responseType: 'arraybuffer',
...
export async function convertToOpus(buffer) {
	...
	const res = await axios.post(`${config.XSTRO_API}/api/convert-to-opus`, formData, {
		headers: {
			...formData.getHeaders(),
		},
		responseType: 'arraybuffer',
...
export const toSticker = async (buffer, packname = config.STICKER_PACK.split(';')[1], author = config.STICKER_PACK.split(';')[0]) => {
	try {
		...
		const res = await axios.post(`${config.XSTRO_API}/api/sticker`, form, {
			headers: {
				...headers,
			},
			responseType: 'arraybuffer',
