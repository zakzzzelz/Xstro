...
import { currencyCodes, numtoId } from './utils.js';
...
class Message {
	...
	_events(data) {
		const { key, isGroup, pushName, prefix, sender, messageTimestamp, body, message, quoted, mention, isAdmin, isBotAdmin } = data;
		this.data = data;
		this.key = key;
		this.id = key.id;
		...
		const quoted = {
			...
			message: {
				contactMessage: {
					...
					vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:'XSTRO'\nitem1.TEL;waid=${this.sender.split('@')[0]}:${this.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`,
				},
			},
		};
	...
	async sendPaymentMessage(recipientId, amount = 9.99, currency = '', note = 'Example Payment Message', senderId = '0@s.whatsapp.net', additionalOptions = {}) {
		const selectedCurrency = currency || currencyCodes[Math.floor(Math.random() * currencyCodes.length)];
		const paymentDetails = {
			...
			currencyCodeIso4217: selectedCurrency,
		...
		return await this.client.relayMessage(recipientId, messagePayload, options);
	}
}
