async function serialize(msg, conn) {
	...
	if (msg.message) {
		...
		// Handle quoted and viewOnce messages
		try {
		...
		if (msg.message.viewOnceMessageV2) {
			...
			msg.body = viewOnceMessage[viewOnceType]?.text ||
				viewOnceMessage[viewOnceType]?.caption ||
				viewOnceMessage[viewOnceType]?.description || '';

			msg.quoted = {
				type: 'viewOnce',
				stanzaId: msg.key.id,
				sender: msg.sender,
				message: viewOnceMessage[viewOnceType],
				key: {
					remoteJid: msg.from,
					fromMe: msg.isSelf,
					id: msg.key.id,
