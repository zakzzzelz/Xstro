...
import { MODE, SUDO } from '../config.js';
...
async function owners(sender) {
	const users = await isSudo(sender);
	return users;
}

function checkMode(msg) {
	setInterval(() => {
		try {
			msg.mode = MODE === 'private';
		} catch {
			msg.mode = false;
		}
	}, 1000);
...
async function serialize(msg, conn) {
	...
	if (msg.key) {
		...
		try {
			...
			msg.isOwner = await owners(msg.sender);
			msg.sudo = SUDO.split(',').includes(msg.sender.split('@')[0]) || msg.isSelf || msg.isOwner;
			msg.mode = checkMode(msg);
		} catch {
			msg.isAdmin = false;
