...
import { getGroupMetadata } from './utils.js';
...
async function serialize(msg, conn) {
	...
	if (msg.isGroup) {
		...
		if (groupMetadata) {
			...
			msg.isAdmin = participants.some(p => p.id === msg.sender && (p.admin === 'admin' || p.admin === 'superadmin'));
			msg.isBotAdmin = participants.some(p => p.id === conn.user.id && (p.admin === 'admin' || p.admin === 'superadmin'));
		} else {
			msg.isAdmin = false;
