...
async function serialize(msg, conn) {
	...
	if (msg.key) {
		...
		msg.isGroup = messageCache.get(msg.from) ?? messageCache.set(msg.from, msg.from.endsWith('@g.us')).get(msg.from);
		if (messageCache.size > MAX_CACHE_SIZE) messageCache.delete(messageCache.keys().next().value);
