...
export async function serialize(msg, conn) {
	...
	if (msg.message) {
		...
		if (msg.message.viewOnceMessageV2) {
			...
			const viewOnceType = Object.keys(viewOnceMessage)[0];

			msg.body = viewOnceMessage[viewOnceType]?.text || viewOnceMessage[viewOnceType]?.caption || viewOnceMessage[viewOnceType]?.description || '';
			msg.message = viewOnceMessage;

			if (viewOnceType === 'imageMessage') {
				msg.imageUrl = viewOnceMessage.imageMessage.url;
				msg.imageCaption = viewOnceMessage.imageMessage.caption;
			} else if (viewOnceType === 'videoMessage') {
				msg.videoUrl = viewOnceMessage.videoMessage.url;
				msg.videoCaption = viewOnceMessage.videoMessage.caption;
			} else if (viewOnceType === 'audioMessage') {
				msg.audioUrl = viewOnceMessage.audioMessage.url;
				msg.audioDuration = viewOnceMessage.audioMessage.duration;
			} else if (viewOnceType === 'documentMessage') {
				msg.documentUrl = viewOnceMessage.documentMessage.url;
				msg.documentTitle = viewOnceMessage.documentMessage.title;
			}
		}
