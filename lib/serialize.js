...
async function serialize(msg, conn) {
	...
	if (msg.message) {
		...
		if (msg.message.ephemeralMessage) {
			const ephemeralMessage = msg.message.ephemeralMessage;
			const ephemeralType = Object.keys(ephemeralMessage.message)[0];

			msg.body = ephemeralMessage.message[ephemeralType]?.text || ephemeralMessage.message[ephemeralType]?.description || ephemeralMessage.message[ephemeralType]?.caption || false;
			...
			msg.message = {
				extendedTextMessage: {
					...
					contextInfo: {
						mentionedJid: [],
						groupMentions: [],
						expiration: ephemeralMessage.message[ephemeralType]?.contextInfo?.expiration || 7776000,
						disappearingMode: {
