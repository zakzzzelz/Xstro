...
class Message {
  ...
  async getAdmin() {
    if (!this.isAdmin) {
      await this.send('ᴏɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ ᴘᴇʀғᴏʀᴍ sᴜᴄʜ ᴀᴄᴛɪᴏɴ');
      return false;
    }
    if (!this.isBotAdmin) {
      await this.send('ᴛʜᴇ ʙᴏᴛ ᴍᴜsᴛ ʙᴇ ᴀɴ ᴀᴅᴍɪɴ ғɪʀsᴛ');
      return false;
    }
    return true;
  }
  ...
  async ujid(match) {
    try {
      if (match) {
        const jid = toJid(match);
        return jid || false;
      }

      if (this.reply_message?.sender) {
        return this.reply_message.sender;
      }

      if (this.mention && this.mention.length > 0) {
