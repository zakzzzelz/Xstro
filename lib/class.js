...
import { getContentType, generateForwardMessageContent, generateWAMessageFromContent, downloadMediaMessage, generateMessageID } from 'baileys';
...
class Message {
	...
	_events(data) {
		const { key, isGroup, pushName, prefix, sender, messageTimestamp, body, message, quoted, mention, isAdmin, isBotAdmin, send, type } = data;
		this.data = data;
		this.key = key;
		this.id = key.id;
	...
	async send(content, opts = {}) {
		...
		const msg = await this.client.sendMessage(jid, { [type]: content, contextInfo: { mentionedJid: mentions, ...contextInfo }, ...opts }, { quoted: quoted });
		return new Message(this.client, msg);
	}
	...
	async sendFile(file, fileName, caption, opts = {}) {
		...
		const data = await this.client.sendMessage(this.jid, { document: file, mimetype: mime, fileName: fileName || 'χѕтяσ м∂', caption: caption, ...opts }, { quoted: opts.quoted, ...opts });
	...
	async sendFromUrl(url, auto = true, opts = {}) {
		...
		if (auto) {
			...
			const data = await this.client.sendMessage(this.jid, { [content]: buffer, ...opts }, { ...opts });
			return new Message(this.client, data);
		} else {
			const sendType = ['text', 'image', 'sticker', 'video', 'document'];
			if (!opts.type || !sendType.includes(opts.type)) throw Error('No valid content type specified');
			buffer = await getBuffer(url);
			const data = await this.client.sendMessage(this.jid, { [opts.type]: buffer, ...opts }, { ...opts });
			return new Message(this.client, data);
		}
	}
