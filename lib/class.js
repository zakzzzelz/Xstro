...
class Message {
  ...
  _events(data) {
    ...
    this.reply_message = data?.quoted
      ? {
          id: data.data.quoted.key.id,
          fromMe: data.quoted.key?.fromMe,
          sender: toJid(data.quoted.key?.participant || data.quoted.key?.remoteJid),
          key: data.quoted.key,
          bot: /^(BAE5|3EB0)/.test(data.quoted.key.id),
          message: data.quoted?.message,
          text: data.quoted.body,
          image: data.quoted?.type === 'imageMessage',
          video: data.quoted?.type === 'videoMessage',
          audio: data.quoted?.type === 'audioMessage',
          sticker: data.quoted?.type === 'stickerMessage',
          document: data.quoted?.type === 'documentMessage',
          viewonce: data.quoted?.viewonce,
        }
      : false;
  }
  ...
  async download(file = false) {
    const buffer = await downloadMediaMessage(
      {
        key: this.data.data.quoted.key,
        message: this.data?.data.quoted?.message,
      },
      'buffer',
      {},
      {
