   ...
   async (message) => {
      if (message.isGroup) {
         ...
         try {
            ...
            const res = await utils.getBuffer(pp);
...
bot(
   ...
   async (message, match) => {
      ...
      const res = await utils.getJson(`${base_url}encrypt?apikey=${API_KEY}&code=${code}`);
...
bot(
   ...
   async (message, match) => {
      ...
      const res = await utils.getJson(`${config.BASE_API_URL}/api/shorten?url=${url}`);
