...
bot(
  ...
  async (message, match, m, client) => {
    ...
    await message.sendReply(
      `_Broadcasting to ${groupIds.length} groups. Estimated completion in ${groupIds.length * 1.5} seconds_`
    );
  ...
  async (message, match, m) => {
    ...
    if (!match)
      return message.sendReply(
        `_${message.prefix}antiword on_\n_${message.prefix}antiword off_\n_${message.prefix}antiword set badword1,badword2_`
      );

    if (match === 'on') {
      if (antiWordConfig[0].isEnabled)
        return message.sendReply('_Antiword is already enabled for this group._');
      antiWordConfig[0].isEnabled = true;
      await antiWordConfig[0].save();
      const words = antiWordConfig[0].filterWords;
      return message.sendReply(
        words.length > 0
          ? '_Antiword has been enabled for this group._'
          : '_Antiword is enabled but no bad words were set._'
      );
    }

    if (match === 'off') {
      if (!antiWordConfig[0].isEnabled)
        return message.sendReply('_Antiword is already disabled for this group._');
...
bot(
  ...
  async (message, match, m, client) => {
    ...
    if (!countryCode || isNaN(countryCode))
      return message.sendReply('_Please provide a valid country code._');
    ...
    if (!toKick.length)
      return message.sendReply(`_No members found with the country code ${countryCode}._`);
...
bot(
  ...
  async (message, match, m, client) => {
    ...
    if (metadata.announce)
      return message.sendReply('_Group is already muted. Only admins can send messages._');
...
bot(
  ...
  async (message, match, m, client) => {
    ...
    const metadata = await client.groupMetadata(m.from);
    if (!metadata.announce)
      return message.sendReply('_Group is already unmuted. All members can send messages._');
