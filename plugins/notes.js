...
bot(
	...
	async (message, match) => {
		...
		if (!text)
			return await message.send('*Format*: _.addnote title|content_');
		const [title, content] = text.split('|');
		if (!title)
			return await message.send('*Format*: _.addnote title|content_');
		const words = content ? content.trim().split(/\s+/) : [];
		if (words.length > 500)
			return await message.send('*Content exceeds 500 words limit*');
...
bot(
	...
	async (message, match) => {
		...
		if (!noteId || isNaN(noteId))
			return await message.send('*Format*: _.delnote id_');
	...
	async (message, match) => {
		...
		if (!text)
			return await message.send(
				'*Format*: _.editnote id; title|content_',
			);
		...
		const updatedNote = await updateNote(parseInt(id), updates).catch(
			() => null,
		);
		if (!updatedNote)
			return await message.send('*Unable to perform action*');
...
bot(
	...
	async (message, match) => {
		const noteId = parseInt(match.trim());
		if (!noteId || isNaN(noteId))
			return await message.send('*Format*: _.cnote id_');
		const notes = await getNotes().catch(() => null);
		if (!notes) return await message.send('*Unable to perform action*');
