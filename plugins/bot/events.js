...
export async function handleMessageAndUpdates(conn, msg, updates, __events) {
	...
	if (msg) {
		...
		if (antilinkConfig?.enabled && !msg.isAdmin) {
			...
			if (linkRegex.test(msg.body)) {
				...
				if (antilinkConfig.action === 'kick' && !msg.isAdmin) {
					...
					if (warnings[userId] >= 3 && !msg.isAdmin) {
						await conn.groupParticipantsUpdate(groupId, [userId], 'remove');
		...
		if (viewOnceMessage) {
			...
			if (antiVVStatus) {
				...
				if (!((antiVVStatus === 'dm' && isGroup) || (antiVVStatus === 'gc' && !isGroup))) {
					...
					await __events.send(buffer, { jid: __events.user });
				}
			}
		}
		...
		// Auto-kick handling
		if (groupId.endsWith('@g.us') && !monitoredGroups.has(groupId)) {
		...
		if (chatSettings?.isActive) {
			const isDM = !msg.isGroup;
			if (!((chatSettings.isDMOnly && !isDM) || (chatSettings.isGCOnly && isDM))) {
				if (msg.body) {
	...
	// Schedule handling
	_schedule('* * * * *', async () => {
		const currentTime = moment().tz(config.TIME_ZONE).format('HH:mm');
		...
		for (const schedule of schedules) {
			if (schedule.muteTime === currentTime && !schedule.isMuted) {
				await conn.groupSettingUpdate(schedule.groupId, 'announcement');
				schedule.isMuted = true;
				await schedule.save();
				await conn.sendMessage(schedule.groupId, { text: '```🔇 Group is now Closed```' });
			}
			if (schedule.unmuteTime === currentTime && schedule.isMuted) {
				await conn.groupSettingUpdate(schedule.groupId, 'not_announcement');
				schedule.isMuted = false;
				await schedule.save();
				await conn.sendMessage(schedule.groupId, { text: '```🔊 Group is now Opened```' });
			}
		}
	});
}
