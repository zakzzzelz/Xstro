...
bot(
  ...
  async (message, match, m, client) => {
    ...
    if (!jid || jid === '')
      return message.sendReply('_Reply, tag, or give me the participant number_');
    ...
    if (response.success) {
      ...
      if (response.warnings > 3) {
        if (message.isGroup) {
          if (m.isAdmin || m.isBotAdmin) {
            try {
              ...
              await message.sendReply(
                `@${jid.split('@')[0]} *was removed from the group and blocked for exceeding the warning limit.*`,
                { mentions: [jid] }
              );
            } catch (error) {
              await message.sendReply(
                `*Failed to remove or block @${jid.split('@')[0]}:* ${error.message}`,
                { mentions: [jid] }
              );
            }
          } else {
            await message.sendReply(
              '_Cannot take action as I am not an admin or the command issuer is not an admin._'
            );
          }
        } else {
          ...
          await resetWarn(jid);
          await message.sendReply(
            `*User @${jid.split('@')[0]} was blocked for exceeding the warning limit.*`,
            { mentions: [jid] }
          );
        }
      }
