...
bot(
	...
	async (message, match, { prefix, pushName }) => {
		if (!match) {
			return message.send(`ᴡᴇʟᴄᴏᴍᴇ ᴍᴇssᴀɢᴇs sᴇᴛᴜᴘ\n\n_${prefix}welcome on_\nᴛᴜʀɴ ᴏɴ ᴡᴇʟᴄᴏᴍᴇ ᴍᴇssᴀɢᴇs\n\n_${prefix}welcome "your message here"_\nsᴇᴛᴜᴘ ᴄᴜsᴛᴏᴍ ᴡᴇʟᴄᴏᴍᴇ ᴍᴇssᴀɢᴇs\n\n_${prefix}welcome off_\nᴅɪsᴀʙʟᴇs ᴡᴇʟᴄᴏᴍᴇ ᴍᴇssᴀɢᴇ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ\n\n*νιѕιт ωιкι тσ ℓєαяи нσω тσ ¢υѕтσмιzє уσυя ωєℓ¢σмє мєѕѕαgє:*\n\n_https://github.com/AstroX11/Xstro/wiki/Welcome-&&-Goodbye-Messages_`);
		}
		if (match === 'on'.toLowerCase()) {
			if (await isWelcomeOn(message.jid)) return message.send(`_Welcome Already on for this Group_`);
			await addWelcome(message.jid, true, null);
			return message.send('_Welcome Messages Enabled, use' + prefix + 'welcome set to customize it_');
		}
		if (match === 'off') {
			if (!(await isWelcomeOn(message.jid))) return message.send(`_Welcome Already Disabled for this Group_`);
			await delWelcome(message.jid);
			return message.send(`_Welcome Messages Disabled for this Group_`);
		}
		if (match !== 'off' && match !== 'on') {
			await message.send(`NOTE: _If your text is very long, setup your long welcome message, and reply it to set as custom welcome message_`);
			await delay(2000);
			await addWelcome(message.jid, true, match);
			return message.send(`_Custom Welcome Message Set_`);
		}
		return message.send(`\`\`\`${pushName} that's not right, use ${prefix}welcome to see command usage\`\`\``);
	},
);
...
bot(
	...
	async (message, match, { prefix, pushName }) => {
		if (!match) {
			return message.send(`ɢᴏᴏᴅʙʏᴇ ᴍᴇssᴀɢᴇs sᴇᴛᴜᴘ\n\n_${prefix}goodbye on_\nᴛᴜʀɴ ᴏɴ ɢᴏᴏᴅʙʏᴇ ᴍᴇssᴀɢᴇs\n\n_${prefix}goodbye "your message here"_\nsᴇᴛᴜᴘ ᴄᴜsᴛᴏᴍ ɢᴏᴏᴅʙʏᴇ ᴍᴇssᴀɢᴇs\n\n_${prefix}goodbye off_\nᴅɪsᴀʙʟᴇs ɢᴏᴏᴅʙʏᴇ ᴍᴇssᴀɢᴇ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ\n\n*νιѕιт ωιкι тσ ℓєαяи нσω тσ ¢υѕтσмιzє уσυя gσσdʙүє мєѕѕαgє:*\n\n_https://github.com/AstroX11/Xstro/wiki/Welcome-&&-Goodbye-Messages_`);
		}
		if (match === 'on') {
			if (await isGoodByeOn(message.jid)) return message.send(`_Goodbye Already on for this Group_`);
			await addGoodbye(message.jid, true, null);
			return message.send('_Goodbye Messages Enabled, use ' + prefix + 'goodbye your message, to customize it_');
		}
		if (match === 'off') {
			if (!(await isGoodByeOn(message.jid))) return message.send(`_Goodbye Already Disabled for this Group_`);
			await delGoodBye(message.jid);
			return message.send(`_Goodbye Messages Disabled for this Group_`);
		}
		if (match !== 'off' && match !== 'on') {
			await message.send(`NOTE: _If your text is very long, setup your long goodbye message, and reply it to set as custom goodbye message_`);
			await delay(2000);
