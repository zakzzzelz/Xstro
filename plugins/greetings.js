  ...
  async (message, match, m) => {
    ...
    if (args[0] === 'set') {
      ...
      return message.sendReply(
        `_Welcome message updated successfully!_\n\n_New Message:_ ${welcomeMessage}`
      );
    }
    ...
    if (args[0] === 'get') {
      ...
      return message.sendReply(
        currentMessage
          ? `_Current Welcome Message:_\n${currentMessage}\n\n_Status:_ ${status ? 'Enabled' : 'Disabled'}`
          : '_No Welcome Message has been set yet._'
      );
    }

    return message.sendReply(
      '_Invalid command. Usage: .welcome [on/off] | .welcome set [message] | .welcome get_'
    );
  }
);
...
bot(
  ...
  async (message, match, m) => {
    ...
    if (args[0] === 'set') {
      ...
      return message.sendReply(
        `_Goodbye message updated successfully!_\n\n_New Message:_ ${goodbyeMessage}`
      );
    }
    ...
    if (args[0] === 'get') {
      ...
      return message.sendReply(
        currentMessage
          ? `_Current Goodbye Message:_\n${currentMessage}\n\n_Status:_ ${status ? 'Enabled' : 'Disabled'}`
          : '_No Goodbye Message has been set yet._'
      );
    }

    return message.sendReply(
      '_Invalid command. Usage: .goodbye [on/off] | .goodbye set [message] | .goodbye get_'
    );
  }
);
