...
import { getSudo, delSudo, addSudo } from '../lib/sudo.js'
import { numberToJID } from '../lib/utils.js'
...
bot({ pattern: 'getsudo', alias: 'listsudo', type: 'user' }, async m => {
    let s = await getSudo()
    if (s === '_No Sudo Numbers_') return m.sendReply('*_Sudo Users_*\n\n_No Sudo Numbers_')
    let list = '*_Sudo Users_*\n\n' + s.split('\n').map((u, i) =>
        `${i + 1}. @${u.replace(/User[s]?:\s*/, '').replace('@s.whatsapp.net', '').trim()}`
    ).join('\n')
    return m.sendReply(list, {
        mentions: s.split('\n').map(u =>
            `${u.replace('@s.whatsapp.net', '').trim()}@s.whatsapp.net`
        )
    })
})