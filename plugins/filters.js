...
bot(
  ...
  async (message, match) => {
    if (!match.includes(';'))
      return await message.send('Use the format: pfilter <text>;<response>');
...
bot(
  ...
  async (message, match) => {
    if (!match.includes(';'))
      return await message.send('Use the format: gfilter <text>;<response>');
...
bot(
  ...
  async (message) => {
    ...
    response += dmFilters.length
      ? dmFilters.map((filter) => `• ${filter.word}: ${filter.response}`).join('\n')
      : 'None';
