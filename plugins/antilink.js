...
bot(
	...
	async (message, match, { prefix }) => {
		...
		if (!message.isAdmin)
			return message.send('```For Group Admins Only!```');
		...
		if (action === 'on') {
			...
			if (existingConfig)
				return message.send('*_Antilink is already on_*');
		...
		if (action === 'set') {
			...
			if (existingConfig && existingConfig.action === setAction)
				return message.send(
					`*_Antilink action is already set to ${setAction}_*`,
				);
		...
		if (action === 'get') {
			...
			return message.send(
				`*_Antilink Configuration:_*\n` +
					...
					`Action: ${
						actionConfig ? actionConfig.action : 'Not set'
					}`,
			);
		}
